  ...
  if submit_button:
    ...
    if st.session_state['new_type'] == 'Order':
        new_pos = new_pos.assign(**{'INITIAL MARGIN': np.where((st.session_state['new_instrument']!='Future') & (st.session_state['new_side']=='Buy'), 0.,
                                                               -1*st.session_state['new_quantity'] * st.session_state['fit_margins'].loc[st.session_state['new_instrument']]['IM'])})
        ...
        new_pos = new_pos.assign(**{'CURRENT BUYING POWER':st.session_state['client_bp'].loc[st.session_state['new_client']]['BUYING POWER'],
                                    'TOTAL REQUIREMENT': np.absolute(np.minimum(np.add(np.add(new_pos['INITIAL MARGIN'], new_pos['PENDING PREMIUM']), new_pos['NLV']), 0))})
...
if 'open_pos' in st.session_state.keys():
    ...
    st.session_state['open_pos'] = st.session_state['open_pos'].assign(**{'MAINTENANCE MARGIN': -1 * np.abs(np.where((st.session_state['open_pos']['SYMBOL']!='Future') & (st.session_state['open_pos']['QUANTITY']>0), 0.,
                                                                         ...
                                                                         'NLV': np.where((st.session_state['open_pos']['SYMBOL']!='Future') & (st.session_state['open_pos']['QUANTITY']<0), 
                                                                                         st.session_state['open_pos']['QUANTITY'] * st.session_state['theor_prices'].loc[st.session_state['open_pos']['SYMBOL'].values[0]]['THEORETICAL PRICE'] * st.session_state['theor_prices'].loc[st.session_state['open_pos']['SYMBOL'].values[0]]['CONTRACT SIZE'],0)})
    if st.session_state['calc_type'] == 'EoD':
